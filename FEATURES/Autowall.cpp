...
namespace FEATURES
{
	namespace RAGEBOT
	{
		...
		Autowall::Autowall_Return_Info Autowall::CalculateDamage(Vector start, Vector end, SDK::CBaseEntity* from_entity, SDK::CBaseEntity* to_entity, int specific_hitgroup)
		{
			...
			// weapon
			auto weapon = reinterpret_cast<SDK::CBaseWeapon*>(INTERFACES::ClientEntityList->GetClientEntityFromHandle((void*)from_entity->GetActiveWeaponIndex()));
			if (!weapon)
				return return_info;
		...
		bool Autowall::CanPenetrate(SDK::CBaseEntity* attacker, Autowall_Info& info, SDK::Weapon_Info* weapon_data)
		{
			...
			int mask = 0x1002;//UTILS::IsWeaponTaser(UTILS::GetWeapon(attacker)) ? 0x1100 : 0x1002;
