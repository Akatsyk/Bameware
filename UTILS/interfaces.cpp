...
namespace INTERFACES
{
	...
	void InitInterfaces()
	{
		...
		MaterialSystem = reinterpret_cast<SDK::IMaterialSystem*>(FindInterface(enc_char("materialsystem.dll"), enc_char("VMaterialSystem")));
		GlowObjManager = reinterpret_cast<SDK::CGlowObjectManager*>(UTILS::FindSignature("client.dll", "A1 ? ? ? ? A8 01 75 4B"));
		...
		MoveHelper = **reinterpret_cast<SDK::IMoveHelper***>((UTILS::FindPattern("client.dll", (PBYTE)"\x8B\x0D\x00\x00\x00\x00\x8B\x46\x08\x68", "xx????xxxx") + 2));
