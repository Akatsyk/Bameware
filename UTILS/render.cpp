...
namespace RENDER
{
	...
	void DrawString(unsigned int Font, int x, int y, CColor color, DWORD alignment, const char* msg, ...)
	{
		va_list va_alist;
		char buf[1024];
		...
		if (alignment & SDK::FONT_RIGHT)
			x -= width;
		if (alignment & SDK::FONT_CENTER)
			x -= width / 2;
		...
		if (alignment & SDK::FONT_OUTLINE)
		{
			INTERFACES::Surface->DrawSetTextColor(CColor(0, 0, 0, color.a()));
