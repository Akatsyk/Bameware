...
#include "../../MAIN/RAGEBOT/BACKTRACK/Backtracking.h"
#include "../../MAIN/RAGEBOT/RESOLVER/Resolver.h"
...
namespace FEATURES
{
	namespace VISUALS
	{
		...
		void VisualsMisc::NoSmoke() {
			if (!smoke1)
				smoke1 = INTERFACES::MaterialSystem->FindMaterial(("particle/vistasmokev1/vistasmokev1_fire"), ("Other textures"));

			if (!smoke2)
				smoke2 = INTERFACES::MaterialSystem->FindMaterial(("particle/vistasmokev1/vistasmokev1_smokegrenade"), ("Other textures"));

			if (!smoke3)
				smoke3 = INTERFACES::MaterialSystem->FindMaterial(("particle/vistasmokev1/vistasmokev1_emods"), ("Other textures"));

			if (!smoke4)
				smoke4 = INTERFACES::MaterialSystem->FindMaterial(("particle/vistasmokev1/vistasmokev1_emods_impactdust"), ("Other textures"));

			if (SETTINGS::main_configs.no_smoke_enabled) {
				if (!smoke1->GetFlag(SDK::MATERIAL_VAR_NO_DRAW))
					smoke1->SetMaterialVarFlag(SDK::MATERIAL_VAR_NO_DRAW, true);

				if (!smoke2->GetFlag(SDK::MATERIAL_VAR_NO_DRAW))
					smoke2->SetMaterialVarFlag(SDK::MATERIAL_VAR_NO_DRAW, true);

				if (!smoke3->GetFlag(SDK::MATERIAL_VAR_NO_DRAW))
					smoke3->SetMaterialVarFlag(SDK::MATERIAL_VAR_NO_DRAW, true);

				if (!smoke4->GetFlag(SDK::MATERIAL_VAR_NO_DRAW))
					smoke4->SetMaterialVarFlag(SDK::MATERIAL_VAR_NO_DRAW, true);
			}

			else {
				if (smoke1->GetFlag(SDK::MATERIAL_VAR_NO_DRAW))
					smoke1->SetMaterialVarFlag(SDK::MATERIAL_VAR_NO_DRAW, false);

				if (smoke2->GetFlag(SDK::MATERIAL_VAR_NO_DRAW))
					smoke2->SetMaterialVarFlag(SDK::MATERIAL_VAR_NO_DRAW, false);

				if (smoke3->GetFlag(SDK::MATERIAL_VAR_NO_DRAW))
					smoke3->SetMaterialVarFlag(SDK::MATERIAL_VAR_NO_DRAW, false);

				if (smoke4->GetFlag(SDK::MATERIAL_VAR_NO_DRAW))
					smoke4->SetMaterialVarFlag(SDK::MATERIAL_VAR_NO_DRAW, false);
			}
		}
