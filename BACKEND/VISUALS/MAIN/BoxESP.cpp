...
namespace FEATURES
{
	namespace VISUALS
	{
		...
		void CBoxESP::DrawBoxInfo(SDK::CBaseEntity* entity, CColor text_color, Vector* corners, bool draw_name, bool draw_weapon, bool draw_resolver)
		{
			...
			auto weapon = INTERFACES::ClientEntityList->GetClientEntityFromHandle((void*)entity->GetActiveWeaponIndex());
			if (!weapon)
				return;
			...
			if (draw_resolver && !(entity->GetIsDormant()) && local_player->GetHealth() > 0)
			{
				...
				text_queue.emplace_back(FEATURES::RAGEBOT::resolver.TranslateResolveRecord(resolve_type), FEATURES::RAGEBOT::resolver.GetResolveColor(FEATURES::RAGEBOT::resolver.GetPlayerResolveInfo(entity).resolve_type));
				...
				text_queue.emplace_back("LBY: " + std::to_string(entity->GetLowerBodyYaw()), CColor(WHITE));
			}
			...
			for (int i = 0; i < text_queue.size(); i++)
			{
				...
				RENDER::DrawF(top_right.x + 4, top_right.y + (i * 11), FONTS::visuals_esp_font_small, false, false, col, text_queue[i].first);
			}
