...
#include "../MAIN/RAGEBOT/ANTIAIM/AntiAim.h"
#include "../MAIN/RAGEBOT/BACKTRACK//Backtracking.h"
#include "../MAIN/RAGEBOT/AUTOWALL/Autowall.h"
...
namespace FEATURES
{
	...
	//		local_player->SetAbsOrigin(last_networked_origin);
		...
		void FixMove(SDK::CUserCmd* cmd, Vector wish_angles) {
			...
			if (local_player->GetMovetype() != MOVETYPE_LADDER)
		...
		void Fakelag(SDK::CUserCmd* cmd)
		{
			...
			if (net_channel->m_choked_packets >= min(14, GLOBAL::choke_amount) || (net_channel->m_choked_packets > 0 && GLOBAL::disable_fake_lag && !GLOBAL::is_fakewalking))
			{
			...
			auto SetClanTag = [](const char* tag, const char* name)
				{
					...
					static auto set_clan_tag = reinterpret_cast<SetClanTagFn>(UTILS::FindPattern(enc_char("engine.dll"), (PBYTE)enc_char("\x53\x56\x57\x8B\xDA\x8B\xF9\xFF\x15"), enc_char("xxxxxxxxx")));
			...
			auto DoBamewareInto = [screen_width, screen_height]()
				{
					...
					{
						static constexpr char bameware_text[] = " B a m e w a r e ";
